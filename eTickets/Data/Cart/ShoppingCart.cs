...
namespace eTickets.Data.Cart
{
    public class ShoppingCart
    {
        ...
        {
            // Get the IHttpContextAccessor from the service provider
            var httpContextAccessor = service.GetService<IHttpContextAccessor>();

            // Ensure that HttpContext and its Session are available
            ...
            // Get AppDbContext from the service provider
            var context = service.GetService<AppDbContext>();

            // Ensure AppDbContext is available
            if (context == null)
            {
                throw new InvalidOperationException("AppDbContext is not available.");
            }

            // Get or create the cartId
            string cartId = session.GetString("CartId") ?? Guid.NewGuid().ToString();
            session.SetString("CartId", cartId);

            // Create and return the ShoppingCart instance
